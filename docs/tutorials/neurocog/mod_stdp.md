...
which will run all three of our models simultaneously for `200` simulated milliseconds and collect statistics of interest. We may then finally make several plots of what happens under each STDP mode (reproducing some key results in <b>[1]</b>. First, we will plot the resulting  synaptic magnitude over time, like so: 
