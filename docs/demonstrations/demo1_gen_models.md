...
Next, in `examples/demo1/`, observe the provided script `sim_train.py`, which contains the
...
(notice for each model there one subfolder in `/examples/demo1/` for each of the
...
that we have also provided in `/examples/demo1/`, which will just simply execute the above
...
training script retrieves from the `examples/data/mnist/` folder (that you unzipped earlier)
...
check inside each of the model folders, i.e., `examples/demo1/gncn_t1/`, `examples/demo1/gncn_t1_sigma/`,
and `examples/demo1/gncn_pdh/`, and see that your script(s) saved/serialized to disk a
few useful files:
...
```
and you will now find inside the folder `examples/demo1/gncn_t1/` a new numpy array
...
directory `examples/demo1/gncn_t1/` that you have a de-serialized learned
prior `prior0.gmm`.
...
The final script provided for you, i.e., `examples/demo1/eval_logpx.py`, will
