...
def initialize_params(dkey, init_kernel, shape, use_numpy=False):
    ...
    if block_diag_mask_width is not None:
        ...
        delete_vals = jnp.zeros((block_diag_mask_width, params.shape[1] - block_diag_mask_width))
        _block_row = jnp.concatenate((keep_vals, delete_vals), axis=1)
        n_repeat = int(params.shape[1]/block_diag_mask_width)
        block_mask = []
